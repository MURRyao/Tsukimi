...
enum NotchGeometryService {
    ...
    static func geometry(for screen: NSScreen) -> NotchGeometry {
        ...
        let visualNotchWidth = hasHardwareNotch ? hardwareNotchWidth : BagEndDesign.Shelf.notchWidth

        let compactWidth = max(BagEndDesign.Shelf.handleWidth + 44, visualNotchWidth + 44)
        let compactHeight = BagEndDesign.Shelf.handleHeight
        let compactFrame = NSRect(
        ...
        let expandedWidth = min(BagEndDesign.Shelf.width, screenFrame.width - 32)
        let expandedHeight = BagEndDesign.Shelf.height
        let expandedFrame = NSRect(
            x: screenFrame.midX - expandedWidth / 2,
            y: screenFrame.maxY - expandedHeight - BagEndDesign.Shelf.expandedTopInset,
            width: expandedWidth,
